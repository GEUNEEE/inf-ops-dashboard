...
RAW_COL_QTY     = 12  # 수량
RAW_COL_PRODUCT = 15  # 제품 (흑염소/화장품/...) — 정산은 흑염소만 대상
DEFAULT_PRODUCT = "흑염소"


def _is_settlement_product(row) -> bool:
    """Raw_Data 행이 정산 대상 제품(흑염소)인지. 제품 컬럼이 비었으면(구형 데이터) 흑염소로 간주."""
    if len(row) > RAW_COL_PRODUCT and row[RAW_COL_PRODUCT]:
        return str(row[RAW_COL_PRODUCT]).strip() == DEFAULT_PRODUCT
    return True
...
def get_month_orders(rawdata_ws, target_month: str, name_map: dict = None) -> list:
    ...
    for row in rawdata_ws.iter_rows(min_row=2, values_only=True):
        if not row or row[0] is None:
            continue
        if not _is_settlement_product(row):
            continue  # 비흑염소 제품은 흑염소 정산서에서 제외
...
def add_logo(ws):
    """초방리 마을 로고를 4월 정산 시트와 동일한 크기/위치로 삽입 (TwoCellAnchor)"""
    ...
    anchor._from = AnchorMarker(col=1, colOff=133350, row=16, rowOff=85725)
    anchor.to    = AnchorMarker(col=2, colOff=1771650, row=26, rowOff=0)
    img.anchor = anchor
    ws.add_image(img)
...
def main():
    ...
    for tmpl_sheet, ytber_name in dynamic_map:
        ...
        cum_before = 0
        unit_price = None
        unit_price_row = find_label_row(ws, "정산 단가") if not is_general else None
        if not is_general:
            cum_before = get_cumulative_qty_before(rawdata_ws, ytber_name, settlement_month, name_map)
            if unit_price_row:
                ws.cell(unit_price_row, 3).value = get_tier_price(cum_before, tier_pricing)
        ...
        if not is_general:
            ...
            if unit_price_row:
                ws.cell(unit_price_row, 3).value = unit_price
        else:
