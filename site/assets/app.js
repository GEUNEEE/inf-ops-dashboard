...
(function () {
  ...
  function historyToSummary(influencers, settlementSummary) {
    ...
    for (const [name, d] of Object.entries(influencers)) {
      ...
      s[name] = {
        ...
        '협찬원가':  cs['협찬원가'] ?? null,
        '체험월목록': cs['체험월목록'] || [],
        // 당월 체험(협찬원가) 발생 여부 — 주문 없어도 카드 표시 조건에 사용
        '당월체험비': d.sponsor_cost_this_month || 0,
      };
    }
    return s;
  }
  ...
  async function selectMonth(btn) {
    ...
    // 월별 퍼널은 메일 시트 월별 값만 사용 (해당 월 데이터 없으면 0).
    // 스냅샷 누적값으로 폴백하지 않음 — 월 필터에 누적치가 새어드는 버그 방지.
    updateKPIs(
      { gross_revenue: h.gross_revenue, net_profit: h.net_profit,
        order_count: h.order_count, unit_count: h.unit_count },
      { reply_rate: mf.reply_rate || 0,
        exp_rate:   mf.exp_rate   || 0,
        ad_rate:    mf.ad_rate    || 0,
  ...
  function renderInfluencerGrid(summary, month) {
    ...
    if (month) {
      // 월별 필터: 해당 월에 수량이 있거나 당월 체험(협찬원가)이 잡힌 인플루언서 표시
      items = items.filter(d => (d['수량'] || 0) > 0 || (d['당월체험비'] || 0) > 0);
    }
